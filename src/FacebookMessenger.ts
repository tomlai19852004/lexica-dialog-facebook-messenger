...
class FacebookMessenger implements Messenger<Request, Response> {
	...
	public request(request: Request): BotRequest {
		...
		if (!isNil(messaging) && !isNil(messaging.message)) {
			...
			if (
				messaging.message.hasOwnProperty("text") ||
	...
	public response(
		...
		const facebookResponses = responses.toArray().map(response => {
	...
	public async send(
		...
		for (const response of responses.toArray()) {
			await req({
				...
				uri: `${configs.get(CONFIG_FACEBOOK_API_BASE_URL)
					.value as string}/me/messages`
			});
		}
	}
}
