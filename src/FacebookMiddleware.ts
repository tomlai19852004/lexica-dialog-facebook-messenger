...
const facebookMiddleware: Middleware = async (context, next) => {
	...
	if (messenger.name === MESSENGER_NAME) {
		...
		if (uniConfigs.has(CONFIG_FACEBOOK_VERIFY_TOKEN)) {
			verifyToken = (uniConfigs.get(CONFIG_FACEBOOK_VERIFY_TOKEN).value as string);
		} else {
			throw new Error('UNI Config not found: ' + CONFIG_FACEBOOK_VERIFY_TOKEN);
		}

		if (serverContext.method === 'GET'
			...
			next()
				.catch(error => logger.error('Error catched by Facebook middleware', rawRequest, error));
		} else {
			await next();
		}
